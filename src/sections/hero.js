import Image from 'components/image';
import {
  Box,
  Container,
  Flex,
  Heading,
  Text,
  Image as Img,
} from 'theme-ui';
import { Link, NewTab } from 'components/link';
import heroImage from 'assets/app/hero.png';
import icon1 from 'assets/app/hero-1.svg';
import icon2 from 'assets/app/hero-2.svg';
import icon3 from 'assets/app/hero-3.svg';
import button1 from 'assets/app/wallet-apple.png';
import button2 from 'assets/app/wallet-google.png';
import button3 from 'assets/app/pwa.png';

const OUR_WALLET_DATA = {
  heroThumb: heroImage,
  title: 'Your growth starts here.',
  text:
    'accountable is a habit tracker and health / wellness app coming to iOS and Android soon. #getaccountable',
  tagLine: '*all platforms coming soon',
  options: [
    // {
    //   icon: icon2,
    //   label: 'Sleep Smarter - Using REM cycles, you can optimize your sleep schedule to always feel rested',
    // },
    // {
    //   icon: icon3,
    //   label: 'Pro Version - Remove ads and be the first to receive new features',
    // },
    // {
    //   icon: icon1,
    //   label: 'Secure - We don\'t keep any data and there are no user accounts',
    // },
  ],
  buttons: [
    {
      icon: button1,
      label: 'App Store',
      link: 'javascript:;',
    },
    {
      icon: button2,
      label: 'Play Store',
      link: 'javascript:;',
    },
    {
      icon: button3,
      label: 'Launch PWA',
      link: 'javascript:;',
    },
  ],
};
const Hero = () => {
  const {
    title,
    text,
    tagLine,
    options,
    buttons,
  } = OUR_WALLET_DATA;
  return (
    <Box sx={{ variant: 'section.hero' }} id="hero">
      <Container sx={styles.container}>
        <Flex sx={styles.row}>
          <Box sx={styles.thumbWrap}>
            <Image
              src={heroImage}
              alt="App Mockup"
              width={615}
              height={770}
            />
          </Box>
          <Box sx={styles.content}>
            <Heading as="h2">{title}</Heading>
            <Text as="p">{text}</Text>
            <Box as="ul" sx={styles.lists}>
              {options.map(({ icon, label }, index) => (
                <Text as="li" key={`list-key-${index}`}>
                  <span>
                    <Img src={icon} alt="icon image" />
                  </span>
                  {label}
                </Text>
              ))}
            </Box>
            <Box sx={styles.buttonWrap}>
              {buttons.map(({ icon, link, label }, index) => (
                <Link
                  sx={styles.button}
                  path={link}
                  key={`button-key-${index}`}
                >
                  <Img src={icon} alt="button icon" />
                  {label}
                </Link>
              ))}
            </Box>
            <Text as="span">{tagLine}</Text>
          </Box>
        </Flex>
      </Container>
    </Box>
  );
};

export default Hero;

const styles = {
  container: {
    margin: 'auto',
  },
  row: {
    pb: ['65px', '75px', null, '92px', '115px', null, '135px', '150px'],
    display: 'flex',
    flexWrap: 'wrap',
    alignItems: 'center',
    flexDirection: ['column-reverse', null, null, 'row'],
    px: [0, null, '40px', 0, null, null, null, '40px'],
  },
  thumbWrap: {
    width: [
      '100%',
      null,
      '520px',
      'calc(100% - 360px)',
      'calc(100% - 480px)',
      'calc(100% - 520px)',
    ],
    mt: ['40px', '45px', null, 0],
    textAlign: ['center'],
    mx: [null, null, 'auto', 0],
  },
  content: {
    maxWidth: ['100%', null, '520px', '360px', '480px', '520px'],
    pl: [0, null, null, '35px', '50px', '80px', '100px', '50px'],
    textAlign: ['center', null, null, 'left'],
    mx: [null, null, 'auto', 0],
    h2: {
      color: 'heading',
      letterSpacing: '-1.2px',
      fontSize: ['24px', '28px', '32px', null, '36px', '40px', '44px', '48px'],
      lineHeight: [1.4, null, 1.35, null, null, 1.25],
      fontWeight: 'body',
      fontFamily: 'archivo',
    },
    p: {
      fontSize: ['15px', null, '16px'],
      lineHeight: [1.9, null, 2.12],
      color: 'body',
      mt: ['12px', '15px'],
      mb: ['18px', '22px', '25px', '20px', '25px', '30px', '40px'],
    },
    span: {
      color: 'text_secondary',
      fontSize: '14px',
    },
  },
  lists: {
    m: 0,
    p: 0,
    listStyle: 'none',
    mb: ['25px', null, '32px', null, '40px', '45px'],
    maxWidth: ['320px', '370px', null, '100%'],
    mx: 'auto',
    li: {
      display: 'flex',
      alignItems: 'center',
      position: 'relative',
      lineHeight: '1.75',
      fontSize: ['14px', null, '15px', null, '16px'],
      color: 'body',
      mb: [3, null, null, null, '20px'],
      span: {
        display: 'flex',
        flexShrink: 0,
        width: ['40px', null, null, null, '50px'],
        lineHeight: '35px',
      },
    },
  },
  buttonWrap: {
    alignItems: 'center',
    justifyContent: ['center', null, null, 'flex-start'],
    ml: [-2, null, '-15px'],
    mb: [2, 3, null, null, null, '22px'],
  },
  button: {
    width: '190px',
    display: 'inline-flex',
    mb: '15px',
    textDecoration: 'none',
    alignItems: 'center',
    justifyContent: 'center',
    color: '#FFFFFF',
    fontWeight: 700,
    backgroundColor: '#161718',
    fontFamily: 'DM Sans',
    borderRadius: '5px',
    padding: ['11px 11px', null, '13px 13px', '11px 11px', '13px 13px'],
    fontSize: ['14px', null, '15px', '14px', '15px'],
    transition: 'all 300ms ease',
    ml: [2, null, 3],
    '>img': {
      mr: '10px',
    },
    '&:hover': {
      opacity: 0.8,
    },
  },
};
